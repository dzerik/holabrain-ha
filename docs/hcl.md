...
| Water heater | `0xE2` | `51020ED8` | Terma AquaPro WiFi | ❓ reported | Entities populate on `thingProtocol` 2 once the ALT-endpoint signing fix lands, and `heating`/`standby`/`keep_warm` tracks the unit correctly. `current_temperature` is wrong: it reports the setpoint (60 °C) rather than the tank's actual temperature (58 °C on the panel while still heating toward it) — looks like `current_temperature` and `temperature` are sourced from the same status key. The unit also has a legionella-disinfect mode and a "smart" mode that learns usage and sets its own target; `operation_list` currently offers `normal`/`eco`/`smart`/`high_temp` — legionella-disinfect has no match. Command effect (changing target temperature/mode from Home Assistant) not yet tried. | reporter, own hardware |
