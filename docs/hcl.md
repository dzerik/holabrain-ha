...
| Water heater | `0xE2` | `51020ED8` | — | ❓ reported | Entities populate on `thingProtocol` 2 once the ALT-endpoint signing fix lands. Temperature reads ~2 °C above the unit's own panel (58 °C on the panel vs. 60 °C reported) and the panel's "dual" mode is not represented in `operation_list` (`normal`/`eco`/`smart`/`high_temp`) — status arrives, but the value mapping needs a second look before this is ✅. Command effect (changing target temperature/mode from Home Assistant) not yet tried. | reporter, own hardware |
