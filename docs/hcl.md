...
| Water heater | `0xE2` | `51020ED8` | Terma AquaPro WiFi | ❓ reported | Entities populate on `thingProtocol` 2 (ALT-endpoint signing fix). `current_temperature`/`temperature`/`heating`-`standby`-`keep_warm` all confirmed against the unit: setpoint was changed on the real appliance and the reported values tracked it correctly, including which raw field is which (`temp` = setpoint, `targetTemp` = measured tank temperature — swapped from what the names suggest). `remaining_time`, a `fault` problem sensor and a diagnostic `tank_configuration` (single/double) sensor added from the same payload. Not yet confirmed: writing a command *from* Home Assistant back to the appliance, and the push channel. The unit also has a legionella-disinfect mode with no match in `operation_list` (`normal`/`eco`/`smart`/`high_temp`) — not implemented, needs its own dump. | reporter, own hardware |
