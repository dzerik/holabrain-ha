...
| Water heater | `0xE2` | `51020ED8` | Terma AquaPro WiFi | ❓ reported | Entities populate on `thingProtocol` 2 (ALT-endpoint signing fix); this was the only appliance on the reporter's account, so protocol 2 is the sole value the routing change has hardware evidence for. `current_temperature` and `temperature` were checked against the unit, including which raw field is which (`temp` = setpoint, `targetTemp` = measured tank temperature — swapped from what the names suggest). This unit does not report `cur_temperature` at all; that key is kept first in the lookup order for the rest of the family, unconfirmed. The whole heating-status enum was observed: the unit cycles `heating` ↔ `keep_warm` (`heatStatus` 2) many times a day over 72 h of history, and `standby` when off. `remaining_time` climbs to roughly 57 minutes and counts back down once per cycle, consistent with two tanks heating in sequence, and goes unknown at `keep_warm` — which is what the `meaningful_when` gate expects. `faultCode` has only ever read `"0"` on this unit, so the fault sensor's active value is still unobserved. `operation_list` is `single`/`double`/`smart`, matching the app's real "Model" picker rather than the generic category's `eco`/`high_temp` flags, and a mode set **from Home Assistant has been confirmed to reach the appliance** — see [below](#the-51020ed8-model-picker) for the evidence. Selecting Smart from Home Assistant writes only `cloudSmart`, and the appliance clears `bodyNum` to `0` itself. Setting a temperature outside Smart (60 → 61 °C in Double) was accepted by the cloud without error, though the physical panel was not checked. Not yet confirmed: that a temperature write shows on the panel, the push channel, and disinfect/`highTemp`, which stays out of `operation_list` entirely. | reporter, own hardware — [#3](https://github.com/dzerik/holabrain-ha/pull/3) |
